...
import { Tab, CreateTabRequest, AddProductRequest, RemoveProductRequest, AddPaymentRequest } from "../types/tab";
...
class TabService {
  ...
  async addPaymentToTab(tabId: string, paymentData: AddPaymentRequest): Promise<Tab> {
    try {
      const endpoint = API_ENDPOINTS.TABS.ADD_PAYMENT.replace(":id", tabId);
      const response = await apiService.post<Tab>(endpoint, paymentData);

      if (response.success && response.data) {
        return response.data;
      }

      throw new Error("Erro ao adicionar pagamento à comanda");
    } catch (error) {
      ...
      const apiError = error as ApiError;
      throw new Error(apiError.message || "Erro ao adicionar pagamento");
    }
  }
}
